...
void Anchor::Init() {
  ...
  velocity_ = params_->anchor.velocity;
  max_velocity_ = velocity_;
  bond_lambda_ = 0;
  mesh_lambda_ = 0;
  active_ = false;
  diffuse_ = (params_->anchor.diffusion_flag ? true : false);
  anchor_.k_spring_ = params_->anchor.k_spring;
  anchored_ = false;
...
void Anchor::UpdatePosition() {
  ...
  if (bound_) {
    ...
    bond_lambda_ = mesh_lambda_ - bonds_[0].first->GetBondNumber()*bond_length_ ;
...
void Anchor::DiffuseBound() {
  ...
  else if (kick > 0 && bond_lambda_ + dr_mag > (bond_length_ = bonds_[0].first->GetLength()) && kick>0) {
    //Move to next bond if it's there
    if (same_bond = !SwitchBonds(true, dr_mag - (bond_length_-bond_lambda_))) {
      // Otherwise move to head of bond
      mesh_lambda_ += bond_length_ - bond_lambda_;
