...
void CrosslinkManager::InitSpecFile() {
  ...
  ospec_file_.write(reinterpret_cast<char*> (&params_->crosslink.n_spec), sizeof(int));
...
bool CrosslinkManager::InitSpecFileInputFromFile(std::string spec_file_name) {
  ...
  if (n_steps != params_->n_steps || n_spec != params_->crosslink.n_spec || delta != params_->delta) {
...
void CrosslinkManager::LoadFromCheckpoints() {
  ...
  if (!params_->crosslink.checkpoint_flag) {
    error_exit("Checkpoint file %s not available for parameter file!", checkpoint_file_.c_str());
  }
  ReadCheckpoints();
  InitOutputFiles();
}
