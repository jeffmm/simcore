...
void XlinkKMCV2::Init(space_struct *pSpace,
  ...
  // eps effective
  switch (node["concentration_0_1"].Type()) {
    case YAML::NodeType::Scalar:
      eps_eff_0_1_[0] = eps_eff_0_1_[1] =
        0.5 * node["concentration_0_1"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      eps_eff_0_1_[0] = node["concentration_0_1"][0].as<double>();
      eps_eff_0_1_[1] = node["concentration_0_1"][1].as<double>();
      break;
  }
  switch (node["concentration_1_2"].Type()) {
    case YAML::NodeType::Scalar:
      eps_eff_1_2_[0] = eps_eff_1_2_[1] =
        0.5 * node["concentration_1_2"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      eps_eff_1_2_[0] = node["concentration_1_2"][0].as<double>();
      eps_eff_1_2_[1] = node["concentration_1_2"][1].as<double>();
      break;
  }

  // on rates
  switch (node["on_rate_0_1"].Type()) {
    case YAML::NodeType::Scalar:
      on_rate_0_1_[0] = on_rate_0_1_[1] =
        node["on_rate_0_1"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      on_rate_0_1_[0] = node["on_rate_0_1"][0].as<double>();
      on_rate_0_1_[1] = node["on_rate_0_1"][1].as<double>();
      break;
  }
  switch (node["on_rate_1_2"].Type()) {
    case YAML::NodeType::Scalar:
      on_rate_1_2_[0] = on_rate_1_2_[1] =
        node["on_rate_1_2"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      on_rate_1_2_[0] = node["on_rate_1_2"][0].as<double>();
      on_rate_1_2_[1] = node["on_rate_1_2"][1].as<double>();
      break;
  }

  // end pause
  switch (node["end_pause"].Type()) {
    case YAML::NodeType::Scalar:
      end_pause_[0] = end_pause_[1] =
        node["end_pause"].as<bool>();
      break;
    case YAML::NodeType::Sequence:
      end_pause_[0] = node["end_pause"][0].as<bool>();
      end_pause_[1] = node["end_pause"][1].as<bool>();
      break;
  }

  // stall force
  switch (node["stall_force"].Type()) {
    case YAML::NodeType::Scalar:
      f_stall_[0] = f_stall_[1] =
        node["stall_force"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      f_stall_[0] = node["stall_force"][0].as<double>();
      f_stall_[1] = node["stall_force"][1].as<double>();
      break;
  }

  // velocity
  switch (node["velocity"].Type()) {
    case YAML::NodeType::Scalar:
      velocity_[0] = velocity_[1] =
        node["velocity"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      velocity_[0] = node["velocity"][0].as<double>();
      velocity_[1] = node["velocity"][1].as<double>();
      break;
  }

  // velocity polar scale
  switch (node["velocity_polar_scale"].Type()) {
    case YAML::NodeType::Scalar:
      velocity_p_scale_[0] = velocity_p_scale_[1] =
        node["velocity_polar_scale"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      velocity_p_scale_[0] = node["velocity_polar_scale"][0].as<double>();
      velocity_p_scale_[1] = node["velocity_polar_scale"][1].as<double>();
      break;
  }

  // velocity antipolar scale
  switch (node["velocity_antipolar_scale"].Type()) {
    case YAML::NodeType::Scalar:
      velocity_ap_scale_[0] = velocity_ap_scale_[1] =
        node["velocity_antipolar_scale"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      velocity_ap_scale_[0] = node["velocity_antipolar_scale"][0].as<double>();
      velocity_ap_scale_[1] = node["velocity_antipolar_scale"][1].as<double>();
      break;
  }

  // velocity switch costheta
  switch (node["velocity_switch_costheta"].Type()) {
    case YAML::NodeType::Scalar:
      velocity_switch_costheta_[0] = velocity_switch_costheta_[1] =
        node["velocity_switch_costheta"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      velocity_switch_costheta_[0] = node["velocity_switch_costheta"][0].as<double>();
      velocity_switch_costheta_[1] = node["velocity_switch_costheta"][1].as<double>();
      break;
  }

  // Diffusion free
  diffusion_free_ = node["diffusion_free"].as<double>();
  reflect_        = node["reflect"].as<bool>();

  // Diffusion singly bound
  switch (node["diffusion_singly_bound"].Type()) {
    case YAML::NodeType::Scalar:
      diffusion_bound_1_[0] = diffusion_bound_1_[1] =
        node["diffusion_singly_bound"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      diffusion_bound_1_[0] = node["diffusion_singly_bound"][0].as<double>();
      diffusion_bound_1_[1] = node["diffusion_singly_bound"][1].as<double>();
      break;
  }

  // Diffusion doubly bound
  switch (node["diffusion_doubly_bound"].Type()) {
    case YAML::NodeType::Scalar:
      diffusion_bound_2_[0] = diffusion_bound_2_[1] =
        node["diffusion_doubly_bound"].as<double>();
      break;
    case YAML::NodeType::Sequence:
      diffusion_bound_2_[0] = node["diffusion_doubly_bound"][0].as<double>();
      diffusion_bound_2_[1] = node["diffusion_doubly_bound"][1].as<double>();
      break;
  }

  alpha_          = node["alpha"].as<double>();
  rcutoff_0_1_    = node["rcut"].as<double>();
  barrier_weight_ = node["barrier_weight"].as<double>();
  k_stretch_      = node["spring_constant"].as<double>();
  r_equil_        = node["equilibrium_length"].as<double>();
  polar_affinity_ = node["polar_affinity"].as<double>();
  write_event_    = node["write_event"].as<bool>();

  // XXX Check k_stretch and r_equil
  // done the first time we call the potential

  // Stall type
  std::string stall_str = node["stall_type"].as<std::string>();
...
void XlinkKMCV2::GenerateTrackingScheme(YAML::Node *subnode) {
  ...
  YAML::Node node_original = *subnode;
