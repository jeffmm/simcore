...
void FilamentSpecies::InitGNFAnalysis() {
  std::string fname = params_->run_name;
  fname.append("_filament.gnf");
  gnf_file_.open(fname, std::ios::out);
  gnf_file_ << "gnf analysis file\n";
  gnf_file_ << "length diameter bond_length persistence_length umax driving "
  ...
  for (int j = 0; j < n_boxes; ++j) {
    gnf_file_ << box_rad * params_->system_radius << " ";
    box_rad += box_rad;
  }
  gnf_file_ << "\n";
  gnf_file_ << "time";
...
void FilamentSpecies::RunGNFAnalysis() {
  int n_boxes = sparams_.number_fluctuation_boxes;
  ...
  for (int i_center = 0; i_center < n_centers; ++i_center) {
    ...
    for (int i_box = 0; i_box < n_boxes; ++i_box) {
      gnf_file_ << " " << box_n_[i_box];
    }
  }
  gnf_file_ << "\n";
}

void FilamentSpecies::FinalizeGNFAnalysis() { delete[] box_n_; }
