...
void OutputManager::Init(system_parameters *params, 
  ...
  //tot_virial_.resize(3, std::vector<double>(3,0));
  Clear();
}
  ...
  if ( YAML::Node thermo = node_["thermo"] ){
    if (*i_step_ % thermo["steps"].as<int>() == 0 ) {
      ...
      for (yaml_it it=thermo_spec.begin(); it!=thermo_spec.end(); ++it){
        ...
        //else { 
          //std::string sid_str = it->first.as<std::string>();
          //SID sid = StringToSID(sid_str);
          //if (sid != SID::none && species_.count(sid))
            //CalcSpeciesOutputs(sid, &(it->second));
        //}
      }
    }
  }
