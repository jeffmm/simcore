...
void BeadSpringSpecies::InitMse2eAnalysis() {
  ...
  mse2e_file_ << "length diameter bond_length persistence_length driving ndim nsteps nspec delta\n";
  ...
  mse2e_file_ << l << " " << d << " " << cl << " " << pl << " " << dr << " " << params_->n_dim << " " << params_->n_steps << " " << nspec << " " << params_->delta << "\n";
  mse2e_file_ << "num_bead_springs_averaged mse2e_mean mse2e_std_err avg_contour_length theory\n";
  mse2e_ = 0;
  mse2e2_ = 0;
  n_samples_ = 0;
  avg_clen_ = 0;
...
void BeadSpringSpecies::FinalizeMse2eAnalysis() {
  ...
  double theory;
  if (params_->n_dim == 2) {
    theory = avg_clen_ * pl * 4.0 - 8.0 * pl * pl * (1-exp(-0.5*avg_clen_/pl));
  }
  else {
