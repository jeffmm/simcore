...
void BeadSpring::SetAnchor(Anchor * a) {
  anchor_ = a;
  anchored_ = true;
}
...
void BeadSpring::InitElements() {
  ...
  if (length_ > max_length_) {
    warning("BeadSpring length larger than max length -- setting length = max_length");
    length_ = max_length_;
  }
  else if (length_ > 0 && length_ < min_length_) {
    warning("BeadSpring length less than min length -- setting length = min_length");
    length_ = min_length_;
  }
...
void BeadSpring::CalculateBinding() {
...
void BeadSpring::CalculateAngles(bool rescale) {
...
void BeadSpring::ConstructUnprojectedRandomForces() {
...
void BeadSpring::GeometricallyProjectRandomForces() {
  if (!stoch_flag_) return;
...
void BeadSpring::CalculateBendingForces() {
  if (metric_forces_) {
    det_t_mat_[0] = 1;
    det_t_mat_[1] = 2;
...
void BeadSpring::CalculateTensions() {
...
bool BeadSpring::CheckBondLengths() {
...
void BeadSpring::ApplyAnchorForces() {
...
void BeadSpring::DynamicInstability() {
...
void BeadSpring::GrowBeadSpring() {
...
void BeadSpring::UpdatePolyState() {
  double p_g2s = p_g2s_;
  double p_p2s = p_p2s_;
...
void BeadSpringSpecies::InitSpiralAnalysis() {
  std::string fname = params_->run_name;
...
void BeadSpringSpecies::RunSpiralAnalysis() {
