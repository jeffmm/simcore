...
// ); double r_mag = 0.0; for (int i=0;i<n_dim_-1;++i) { r_mag += SQR(r[i]);
//}
// double z0 = (in_mother ? 0 : space_->bud_height);
//[> Check if in_cone_region means what it says it means <]
// if (in_cone_region) {
// double cone_rho2 =
// SQR(space_->bud_neck_radius)*SQR(r[n_dim_-1]-z0)/SQR(space_->bud_neck_height);
...
// if (scale_factor < 0) error_exit("Something went wrong in
// CheckNearBuddingBoundary !\n"); dr_mag2 = 0; for (int i=0;i<n_dim_-1; ++i) {
  ...
  if (bond_lambda_ - dr_mag < 0 && kick < 0) {
    ...
    if (same_bond = !SwitchBonds(false, dr_mag - bond_lambda_)) {
      ...
      bond_lambda_ = 0.0;
             ...
             bond_lambda_ + dr_mag > (bond_length_ = bond_->GetLength()) &&
             kick > 0) {
    // Move to next bond if it's there
    if (same_bond =
            !SwitchBonds(true, dr_mag - (bond_length_ - bond_lambda_))) {
