...
class LJ126 : public PotentialBase {
 ...
 public:
  ...
  LJ126(double pEps, double pSigma, space_struct *pSpace, double pRcut,
        double pFcut)
      : PotentialBase(pSpace, pRcut, pFcut), eps_(pEps), sigma_(pSigma) {
    pot_name_ = "Lennard Jones 12-6";
  ...
  virtual void Print() {
    PotentialBase::Print();
    std::cout << "\t{eps:" << eps_ << "}, {sigma:" << sigma_ << "}, {c6:" << c6_
              << "}, {c12:" << c12_ << "}, {shift:" << shift_ << "}\n";
  }

  virtual void CalcPotential(interactionmindist *idm, Simple *part1,
                             Simple *part2, double *fpote) {
    ...
    if (ABS(ffac) > fcut_) ffac = SIGNOF(ffac) * fcut_;
    for (int i = 0; i < n_dim_; ++i) fpote[i] = dr[i] * ffac;
  ...
  virtual void Init(space_struct *pSpace, int ipot, YAML::Node &node) {
    ...
    // Now, let's look at the particular yaml node we are supposed to be
    // interested in
